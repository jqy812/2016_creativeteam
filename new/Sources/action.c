...
{
	if((site>>8)!=0x25 && (site>>8)!=0x23 && (site>>8)!=0x0A)
...
//		bz=6;
    	delay_ms(1000);
		sending_service_package(0x33,0x00CD,0x0A01);
    	set_steer_helm_basement(data_steer_helm_basement.center);
    	g_f_enable_speed_control=1;
		set_speed_target(-9);
		while(delay_count>-3300)
		{
			LCD_Write_Num(105,3,delay_count,4);
		}	
		g_f_enable_speed_control=0;
		set_speed_pwm(0);
		bz=6;
		sending_service_package(0x33,0x00CD,0x0A02);
		delay_ms(1000);
		Hang_Up();
		delay_ms(1000);
		sending_service_package(0x33,0x00CD,0x0A03);
		bz=-6;
		fieldover=1;
		g_f_enable_speed_control=1;
		velocity=16;
...
//	}
	...
	if((site>>0)==0x8009)
	{
		...
		delay_count=0;
		set_steer_helm_basement(data_steer_helm_basement.right_limit);
		while(delay_count<300)
		{
			LCD_Write_Num(105,3,delay_count,4);
		}
		sending_service_package(0x33,0x00CD,0x0A04);
		bz=4;
	}
}
